...
process.on('unhandledRejection', (reason) => {
    ...
    // ── FIX: Puppeteer protocol timeout → bot colgado, reiniciar ──
    // Ocurre cuando Chrome no responde a un comando dentro del protocolTimeout.
    // Sin este handler el bot queda en estado zombie (waStatus=CONECTADO pero muerto).
    const reasonMsg = reason instanceof Error ? reason.message : String(reason);
    if (reasonMsg.includes('callFunctionOn timed out') || reasonMsg.includes('protocolTimeout') || reasonMsg.includes('Increase the \'protocolTimeout\'')) {
        waLog.add('⏱️ Puppeteer protocol timeout — reiniciando bot en 5s...');
        waStatus = 'DESCONECTADO';
        waReady = false;
        waInitializing = false;
        setTimeout(async () => {
        ...
        return; // No loguear como error crítico, es un timeout transitorio
    }
    // ──────────────────────────────────────────────────────────────
...
// Schema de lectura cruzada: Sites (sitios/clientes) de la app principal
// Los tickets guardan el _id del Site en el campo siteId, NO el folio del proyecto
const SiteRefSchema = new mongoose.Schema({
    _id: { type: String },
    nombre: String,
    ubicacion: String,
    companyId: String
...
const SiteRef = mongoose.model('SiteRef', SiteRefSchema);
...
// =====================================================================
// DIAGNÓSTICO TEMPORAL: Inspeccionar datos de cierre de proyecto
// Endpoint: GET /api/debug/proyecto-cierre/:id
// Muestra tickets, firmas y facturas sin modificar nada.
// =====================================================================
app.get('/api/debug/proyecto-cierre/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const proyecto = await CRMProyecto.findById(id);
        if (!proyecto) return res.status(404).json({ error: 'Proyecto no encontrado', id });

        const folioProyecto = proyecto.folio || '';
        const nombreProyecto = proyecto.nombre || '';
        const nombreCliente = proyecto.clienteNombre || '';

        // -- Paso 1: Resolver Sites por nombre (igual que la validación real) --
        const siteSearchTerms = [folioProyecto, nombreProyecto, nombreCliente].filter(t => t.trim() !== '');
        let sitesEncontrados = [];
        let siteIds = [];
        if (siteSearchTerms.length > 0) {
            const siteOrConditions = siteSearchTerms.map(term => ({
                nombre: { $regex: term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' }
            }));
            sitesEncontrados = await SiteRef.find({ $or: siteOrConditions }).select('_id nombre ubicacion');
            siteIds = sitesEncontrados.map(s => s._id.toString());
        }

        // -- Paso 2: Consulta de tickets con los siteIds resueltos --
        const ticketOrConditions = [
            { proyectoId: id },
            { proyectoId: folioProyecto },
        ];
        if (folioProyecto) {
            ticketOrConditions.push({ siteId: folioProyecto });
            ticketOrConditions.push({ proyectoId: { $regex: folioProyecto.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' } });
        }
        if (siteIds.length > 0) {
            ticketOrConditions.push({ siteId: { $in: siteIds } });
        }

        const allTickets = await TicketRef.find({ $or: ticketOrConditions })
            .select('_id folio siteId proyectoId firmaCliente firmaTecnico estado titulo');

        const ticketFirmado = await TicketRef.findOne({
            firmaCliente: { $exists: true, $ne: null, $ne: '' },
            $or: ticketOrConditions
        }).select('_id folio siteId firmaCliente');

        // -- Info de cotización --
        let cotInfo = null;
        if (proyecto.cotizacionId) {
            const cot = await CRMCotizacion.findById(proyecto.cotizacionId)
                .select('folio total subtotal iva estado');
            cotInfo = cot;
        }

        // -- Suma de facturas guardadas --
        const sumaFacturas = (proyecto.facturas || []).reduce((acc, f) => acc + (f.monto || 0), 0);

        // -- Muestra los primeros 5 tickets de la colección para comparar siteId --
        const sampleTickets = await TicketRef.find({}).limit(5).select('_id folio siteId titulo');
        ...
        res.json({
            ...
            diagnostico: {
                terminosBusquedaSite: siteSearchTerms,
                sitesEncontrados: sitesEncontrados,
                siteIdsResueltos: siteIds,
                totalTicketsEncontrados: allTickets.length,
                ticketFirmadoEncontrado: !!ticketFirmado,
                ticketFirmado: ticketFirmado || null,
                allTickets: allTickets,
                cotizacion: cotInfo,
                sumaFacturas: sumaFacturas,
                coincidenciaFacturas: cotInfo ? Math.abs(sumaFacturas - (cotInfo.total || 0)) <= 1 : null,
                muestraTicketsColeccion: sampleTickets
            }
        });
    } catch (err) {
...
app.put('/api/proyectos/:id', async (req, res) => {
    try {
        ...
        if (data.estado && ESTADOS_CIERRE.includes(data.estado)) {
            ...
            // ─────────────────────────────────────────────────────────────────
            // VALIDACIÓN DE ENTREGABLE FIRMADO
            // Los tickets guardan el _id del Site en `siteId`, NO el folio.
            // Paso 1: Resolver qué Sites están asociados a este proyecto
            //   buscando por folio, nombre de proyecto o nombre de cliente.
            // Paso 2: Usar esos _id de Site en la consulta de tickets.
            // ─────────────────────────────────────────────────────────────────
            const folioProyecto = proyecto.folio || '';
            const nombreProyecto = proyecto.nombre || '';
            const nombreCliente = proyecto.clienteNombre || '';

            // Construir términos de búsqueda para Sites
            const siteSearchTerms = [folioProyecto, nombreProyecto, nombreCliente]
                .filter(t => t.trim() !== '');

            // Buscar Sites cuyo nombre coincida con cualquiera de los términos
            let siteIds = [];
            if (siteSearchTerms.length > 0) {
                const siteOrConditions = siteSearchTerms.map(term => ({
                    nombre: { $regex: term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' }
                }));
                const sitesEncontrados = await SiteRef.find({ $or: siteOrConditions }).select('_id nombre');
                siteIds = sitesEncontrados.map(s => s._id.toString());
            }

            // Construir condiciones $or para tickets:
            // - por siteId (resuelto desde Sites)
            // - por siteId directo como string (folio, por si algún ticket lo guarda así)
            // - por proyectoId directo (fallback legacy)
            const ticketOrConditions = [
                { proyectoId: id },
                { proyectoId: folioProyecto },
            ];
            ...
            const ticketFirmado = await TicketRef.findOne(ticketQuery).select('_id folio firmaCliente siteId');
            ...
            // Segunda Validación: La suma de facturas debe igualar al total de la cotización
            if (proyecto.cotizacionId) {
                const cotizacion = await CRMCotizacion.findById(proyecto.cotizacionId);
                if (cotizacion && (cotizacion.total || cotizacion.total === 0)) {
                    const facturasReales = data.facturas || proyecto.facturas || [];
                    const sumaFacturas = facturasReales.reduce((acc, f) => acc + (f.monto || 0), 0);

                    // Permitir margen de $1 por posibles diferencias de redondeo en decimales
                    if (Math.abs(sumaFacturas - cotizacion.total) > 1) {
                        return res.status(422).json({
                            error: `No se puede cerrar el proyecto. La suma de facturas ($${sumaFacturas.toLocaleString('es-MX', {minimumFractionDigits:2})}) no coincide con el monto total de la cotización ($${cotizacion.total.toLocaleString('es-MX', {minimumFractionDigits:2})}). Verifica que hayas incluido el IVA.`
                        });
                    }
                }
            }
...
const initWhatsApp = () => {
    ...
    // ── FIX: Limpiar carpeta temporal de RemoteAuth antes de iniciar ──
    // Si quedó de un proceso anterior, mkdir falla con EEXIST (error unhandledRejection)
    try {
        const tempSessionDir = require('path').join(WA_DATA_PATH, 'wwebjs_temp_session_nais-crm');
        if (fs.existsSync(tempSessionDir)) {
            fs.rmSync(tempSessionDir, { recursive: true, force: true });
            waLog.add('🧹 Directorio temporal de sesión limpiado antes de init');
        }
        // Re-asegurar que el directorio padre exista
        fs.mkdirSync(WA_DATA_PATH, { recursive: true });
    } catch (cleanErr) {
        waLog.add(`⚠️ No se pudo limpiar directorio temporal: ${cleanErr.message?.substring(0, 80)}`);
    }
    // ─────────────────────────────────────────────────────────────────
    ...
    waClient = new Client({
        ...
        puppeteer: {
            ...
            protocolTimeout: 300000, // ✅ Aumentado a 5 min (Railway tiene CPU lenta al arrancar)
            timeout: 120000
        }
    });
        ...
        if (s.state === 'IDLE') {
            ...
            if (proy.id === 'IND') {
                tareas = await CRMActividad.find({ 
                    $or: [ { proyectoId: null }, { proyectoId: '' }, { proyectoId: { $exists: false } } ],
                    estado: { $ne: 'Completada' }
                });
            } else {
