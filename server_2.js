...
❌ *RECHAZAR* — para declinar la asignación`;
            ...
            if (chatId) {
                ...
                const sessionData = { state: 'WAITING_VEHICLE_CONFIRM', ctx: { txId, vehicleId, mainApiUrl: mainApiUrl || 'https://naisa.newbox.mx' } };
...
const waMessageHandler = async message => {
    try {
        ...
        // Si el mensaje viene de un identificador interno (LID), extraer el número real
        if (From && From.includes('@lid')) {
            ...
            } catch(e) { console.error('Error getting contact from LID', e); }
        }

        if (From && From.includes(':')) {
            From = From.split(':')[0] + '@c.us';
        }
        ...
        let sFromAlt = null;
        if (s.state === 'IDLE') {
            const altFrom = From.startsWith('521') ? From.replace('521', '52') : From.replace(/^52/, '521');
        ...
        // DEBUG LOG TO DB TO SEE EXACTLY WHAT ARRIVES
        try {
            await WaSession.create({
                chatId: 'LOG_' + Date.now() + '_' + Math.floor(Math.random()*1000),
                state: 'DEBUG_INCOMING',
                ctx: { From_Raw: message.from, From_Clean: From, Body: Body, ParsedText: text, ResolvedState: s.state, AltFrom: (sFromAlt ? sFromAlt.state : 'null') }
            });
        } catch(e) {}
        ...
        // --- FLUJO TAREAS ---
            ...
            if (b === '1' || b === 'aceptar' || b === 'acepto' || b === 'si' || b === 'sí') {
                try {
                    const mainApiUrl = s.ctx.mainApiUrl || 'https://naisa.newbox.mx';
                    // Llamar al endpoint principal para confirmar (enciende motor, cambia estado a Prestado)
                    const confirmRes = await fetch(`${mainApiUrl}/api/assignments/vehicle/${s.ctx.txId}/confirm`, {
                        method: 'PUT',
                    ...
                    waLog.add(`❌ Error confirmando vehiculo via WA: ${e.message?.substring(0,60)}`);
                ...
                try {
                    const mainApiUrl = s.ctx.mainApiUrl || 'https://naisa.newbox.mx';
                    // Llamar al endpoint principal para rechazar (devuelve vehículo a Disponible)
                    await fetch(`${mainApiUrl}/api/assignments/vehicle/${s.ctx.txId}/reject`, { method: 'PUT' });
                } catch(e) {
                    waLog.add(`⚠️ Error rechazando vehiculo via WA: ${e.message?.substring(0,60)}`);
                }
                await setSession(From, { state: 'IDLE', ctx: {} });
