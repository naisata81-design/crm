...
app.post('/api/whatsapp/asignacion-vehiculo', async (req, res) => {
    try {
        const { txId, vehicleId, userName, modelo, placas, checklist, checklistNotas, imgReporteDanos } = req.body;
        const allUsers = await UserRef.find();
        ...
        if (u && u.telefono) {
            let kitTxt = '';
            if (checklist) {
                kitTxt = Object.entries(checklist).filter(([k, v]) => v).map(([k]) => k).join(', ');
            }
            if (!kitTxt) kitTxt = 'Ninguno';
            
            let msg = `🚨 *NUEVA ASIGNACIÓN DE VEHÍCULO* 🚨\n\nTe han asignado el vehículo: *${modelo}* (Placas: ${placas || 'S/P'}).\n\n📦 *Kit de herramientas entregado:* ${kitTxt}\n📝 *Notas del Kit:* ${checklistNotas || 'Ninguna'}\n\n`;
            if (imgReporteDanos) msg += `📸 *Imagen del equipo / Daños:* ${imgReporteDanos}\n\n`;
            ...
            if (chatId) {
                waSessions.set(chatId, { state: 'WAITING_VEHICLE_CONFIRM', ctx: { txId, vehicleId } });
            }
        }
