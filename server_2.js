...
const waMessageHandler = async message => {
    try {
        ...
        // 2. Cargar sesión desde MongoDB usando el número normalizado (persiste entre reinicios)
        const s = await getSession(normalizedFrom);
        
        // Cancelación Global (más flexible)
        if (text.includes('cancelar') || text === 'cancela' || text === 'salir' || text === 'reiniciar' || text.includes('abortar')) {
            await setSession(normalizedFrom, { state: 'IDLE', ctx: {} });
        ...
        if (s.state === 'IDLE') {
            ...
            if (esLevantamiento || esCita) {
                ...
                await setSession(normalizedFrom, s);
