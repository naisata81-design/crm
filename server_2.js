...
app.get('/api/debug/proyecto-cierre/:id', async (req, res) => {
    try {
        ...
        // -- Consulta SIN filtro de firma (ver todos los tickets vinculados) --
        const allTicketsQuery = {
            $or: [
                { proyectoId: id },
                { proyectoId: folioProyecto },
                { proyectoId: { $regex: folioProyecto, $options: 'i' } },
                { siteId: folioProyecto },
                { siteId: { $regex: folioProyecto, $options: 'i' } }
            ]
        };

        const allTickets = await TicketRef.find(allTicketsQuery)
            .select('_id folio proyectoId siteId firmaCliente firmaTecnico estado notas');

        // -- Consulta CON filtro de firma (la misma que usa el cierre) --
        const ticketQuery = {
            firmaCliente: { $exists: true, $ne: null, $ne: '' },
            ...allTicketsQuery
        };
        const ticketFirmado = await TicketRef.findOne(ticketQuery)
            .select('_id folio proyectoId siteId firmaCliente');
        ...
        res.json({
            ...
            diagnostico: {
                folioUsadoEnBusqueda: folioProyecto,
