...
const waMessageHandler = async message => {
    try {
        ...
        // --- FLUJO TAREAS ---
            ...
            if (user.ocupado) {
                await reply(`⚠️ Nota: Has seleccionado a ${user.nombre} que está 🔴 Ocupado, pero se le asignará la tarea de todos modos.`);
            }
            ...
            if (b === 'ninguno' || b === 'no' || b === '0') {
                ...
                if (ocupadosSeleccionados.length > 0) {
                    await reply(`⚠️ Nota: Has incluido acompañantes ocupados (${ocupadosSeleccionados.join(', ')}).`);
                }
            }
            ...
            if (b === 'ninguno' || b === 'no' || b === '0') {
                ...
                if (ocupadosSeleccionados.length > 0) {
                    await reply(`⚠️ Nota: Has seleccionado vehículos ocupados (${ocupadosSeleccionados.join(', ')}).`);
                }
            }
