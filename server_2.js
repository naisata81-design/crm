    ...
    try {
        ...
        try {
            ...
            const findPhone = (name) => {
                const u = allUsers.find(x => x.nombre && x.nombre.toLowerCase().includes(name.trim().toLowerCase()));
            ...
            if (Array.isArray(data.asignadoANombre)) {
                encargadoName = data.asignadoANombre.join(', ');
            } else if (typeof data.asignadoANombre === 'string') {
                encargadoName = data.asignadoANombre.split(',').map(s => s.trim()).filter(Boolean).join(', ');
            }
            ...
            if (encargadoName) {
                const telEncargado = findPhone(encargadoName);
                if (telEncargado) {
