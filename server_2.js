...
// El ZIP de sesión de RemoteAuth siempre se escribe en process.cwd().
// En Railway, cwd=/app puede ser read-only. Cambiamos cwd a /tmp para que el ZIP se cree ahí.
const WA_DATA_PATH = path.resolve('/tmp/wwebjs');
try { fs.mkdirSync(WA_DATA_PATH, { recursive: true }); } catch(_) {}
try { process.chdir('/tmp'); } catch(e) { waLog.add('⚠️ No se pudo cambiar cwd a /tmp: ' + e.message); }
...
const initWhatsApp = () => {
    ...
    waClient = new Client({
        authStrategy: new RemoteAuth({
            ...
            dataPath: WA_DATA_PATH  // Ruta absoluta /tmp/wwebjs
        }),
        webVersionCache: {
            type: 'remote',
...
setInterval(async () => {
    if (waStatus === 'CONECTADO' || waStatus === 'ESPERANDO_ESCANEO') {
        // Si ya está conectado o mostrando QR activo, no interrumpir
        waInitTimestamp = Date.now();
        return;
    }
    // Solo reiniciar si sigue en DESCONECTADO (o ERROR_AUTH) sin avanzar
    const minutosSinConectar = (Date.now() - waInitTimestamp) / 60000;
    if (minutosSinConectar >= 3) {
        waLog.add(`🐕 Watchdog: Bot lleva ${Math.floor(minutosSinConectar)}min en estado "${waStatus}". Reiniciando...`);
        waInitTimestamp = Date.now(); // Resetear para no reiniciar en loop inmediato
        try {
            if (waClient) {
                try { await waClient.destroy(); } catch(_) {}
                waClient = null;
