...
waClient.on('message', async message => {
    try {
        ...
        if (!Body || !From || From.includes('@g.us')) return; // ignorar grupos por ahora
        ...
        if (s.state === 'IDLE') {
            ...
            const esCita = text.includes('cita') || text.includes('junta') || text.includes('reunion') || text.includes('reunión') || text.includes('reunin');
