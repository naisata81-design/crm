...
// ==========================================
// INTEGRACION TWILIO WHATSAPP - NAIS BOT
// ==========================================
const twilio = require('twilio');
const waAccountSid = 'AC76afac08d6ea318338f5673ff60d9ae3';
const waAuthToken = '0c4e9b925f1cb17109981790b18ef5f3';
const waClient = twilio(waAccountSid, waAuthToken);
const waTwilioNumber = 'whatsapp:+14155238886';

const waSessions = new Map();

async function sendWhatsAppMessage(to, body) {
    try {
        await waClient.messages.create({
            from: waTwilioNumber,
            to: to,
            body: body
        });
    } catch (e) {
        console.error('Error enviando mensaje de WhatsApp:', e);
    }
}
...
app.post('/api/whatsapp/webhook', async (req, res) => {
    ...
    if (s.state === 'IDLE') {
        ...
        if (esLevantamiento || esCita) {
            ...
            await sendWhatsAppMessage(From, "Y'B ¡Hola! Soy NAIS desde tu CRM. Puedo ayudarte a agendar citas, levantamientos y asignar nuevas tareas directamente desde WhatsApp.\n\n¿Qué deseas hacer?");
        }
        ...
        try {
            ...
            await ev.save();
            await sendWhatsAppMessage(From, `o? ¡Listo! Tu ${s.ctx.type} ha sido agendado en el CRM exitosamente.\n\n*Detalles:*\n?" ${s.ctx.desc}\nY". ${s.ctx.dateStr}\no? ${s.ctx.timeStr}\n\nPara agendar algo más o cancelar, solo escríbeme.`);
        } catch(e) {
    ...
    // --- FLUJO TAREAS ---
        ...
        try {
            ...
            tareasDelDia.forEach(t => {
                ...
                // Si el nombre mencionado coincide parcialmente con alguien ocupado (ej: si escriben 'Daniel' y existe 'Daniel Arévalos')
            ...
            const nTarea = new CRMActividad({
                ...
                asignadoANombre: s.ctx.person, // Pendiente de coincidir con ID real en el futuro, pero el texto sirve para el UI
                estado: 'Pendiente',
    ...
    res.status(200).send('<Response></Response>'); // Confirmar recibo a Twilio
});
