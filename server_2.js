...
waClient.on('qr', (qr) => {
    ...
    console.log('📱 QR generado. Visita /whatsapp/qr en tu navegador para escanearlo.');
});
...
waClient.on('ready', () => {
    ...
    waStatus = 'CONECTADO';
    waReady = true;
    console.log('✅ WhatsApp Web (NAIS Bot) conectado y listo!');
});
...
async function sendWhatsAppMessage(to, body) {
    if (!waReady) {
        console.warn('⚠️ WhatsApp client no está listo aún. Reintentando en 3s...');
    ...
    try {
        ...
        console.error('Error enviando mensaje de WhatsApp:', JSON.stringify(e), e.message, e.stack);
    }
}
...
waClient.on('message', async message => {
    try {
        ...
        if (!Body || !From || From.includes('@g.us')) return; // ignorar grupos
        ...
        // --- FLUJO TAREAS ---
            ...
            await reply(`👤 ¿A quién le asignamos esta tarea?\n(Escribe el nombre del trabajador, ej: Daniel, Jacqueline)`);
            ...
            try {
                ...
                tareasDelDia.forEach(t => {
                    ...
                    if (empleadosExist.some(emp => emp.toLowerCase().includes(personLower))) {
                        conflictos.push(`El empleado "${s.ctx.person}" ya tiene: "${t.descripcion}" (${t.horaInicio} - ${t.horaFin || '?'})`);
                    }
                });

                if (conflictos.length > 0) {
                    await reply(`⚠️ *CONFLICTO DE HORARIO*\n\n${conflictos.join('\n')}\n\n*Tarea no creada.* Prueba con otro horario.`);
                    waSessions.set(From, { state: 'IDLE', ctx: {} });
                    return;
                }
